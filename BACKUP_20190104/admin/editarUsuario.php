...
if(!listarUsuarioPorID($id_usuario)){
	echo 'Error al ejecutar la consulta '.mysqli_error($conn);
	exit;
}else{
  $usuarios = listarUsuarioPorID($id_usuario);
    ...
    <div class="page">
      ...
      <div class="breadcrumb-holder">
        <div class="container-fluid">
          <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="adminUsuario.php">Administrar Usuarios</a></li>
      ...
      <section>
        <div class="container-fluid">
          <!-- Page Header-->
          <header>
            <h1 class="h3 display">Editar Usuario</h1>
          </header>

          <!-- INICIO DE FORMULARIO DE CREACION DE EQUIPO -->
					<form name="formEditarUsuario" method="post" enctype="multipart/form-data" action="action_editarUsuario.php">
					 ...
					 <input class="form-control" id="usuario" name="usuario" type="text" aria-describedby="nameHelp" placeholder="Ingrese usuario" value="<?php echo $row['usuario'];?>">
					 <br>
           ...
           <label for="exampleInputName">Cargo</label>
					 <select class="form-control" id="cargo" name="cargo">
						 <option value="0">Seleccione</option>
             <?php if($row['cargo'] == 1){ ?>
						          		<option value="1" selected="selected">Supervisor Technical Consultant</option>
						          		<option value="2">Technical Consultant</option>
                          <option value="3">Technical Consultant II</option>
						          <?php }elseif($row['cargo'] == 2){ ?>
						          		<option value="1">Supervisor Technical Consultant</option>
						          		<option value="2" selected="selected">Technical Consultant</option>
                          <option value="3">Technical Consultant II</option>
						          <?php } elseif($row['cargo'] == 3){ ?>
						          		<option value="1">Supervisor Technical Consultant</option>
						          		<option value="2">Technical Consultant</option>
                          <option value="3" selected="selected">Technical Consultant II</option>
						          <?php } ?>
					 </select>
					 <br>
           ...
           <?php
		        echo '<img src=img/'.$row['foto'].'
		        	  alt="Imagen Usuario" width="200" height="200" style="border-radius:50%;">';
		        echo '<br>';
		        echo '<br>';
		        echo 'Selecciona una nueva imagen';
           ?>
					 <input class="form-control" id="foto" name="foto" type="file">
					 <br>

			       <button class="btn btn-primary btn-block" class="btn btn-primary btn-block" <input type="submit" name="btn_register" id="btn_register" value="Editar"/>Editar</button>
				     <br>
				     <input type="hidden" name="id_usuario" name="id_usuario" value="<?php echo $id_usuario; ?>">
			    </form>
        </div>
      </section>
